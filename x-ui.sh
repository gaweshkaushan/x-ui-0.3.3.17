...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/gaweshkaushan/x-ui-0.3.3.17/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/gaweshkaushan/x-ui-0.3.3.17/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
